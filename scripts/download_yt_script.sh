...
if [ -z "$2" ]
    then
        ...
        ytdl-sub --dry-run dl --single-video-with-foldername --overrides.tv_show_name "Video" --youtube.video_url $1
    else
        # ytdl-sub dl --single-video "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
        echo "Downloading to $2 folder"
        ytdl-sub --dry-run dl --single-video-with-foldername --overrides.tv_show_name "$2" --youtube.video_url $1
fi
